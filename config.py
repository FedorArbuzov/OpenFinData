YANDEX_API_KEY = 'e05f5a12-8e05-4161-ad05-cf435a4e7d5b'

PATH_TO_DB_DIMA = r'C:\Users\User\PycharmProjects\OpenFinData\kb\knowledge_base.db'
PATH_TO_DB_SERVER = r'C:\Users\imon\Documents\Git\Datatron 2.0\OpenFinData\kb\knowledge_base.db'
PATH_TO_USER_DB_DIMA = r'C:\Users\User\PycharmProjects\OpenFinData\db\users.db'
PATH_TO_USER_DB_SERVER = r'C:\Users\imon\Documents\Git\Datatron 2.0\OpenFinData\db\users.db'

PATH_TO_SOLR_POST_JAR_FILE_DIMA = r'"C:\Users\User\Desktop\solr\solr-6.3.0\solr-6.3.0\example\exampledocs\post.jar"'
PATH_TO_SOLR_POST_JAR_FILE_SERVER = r'"C:\local\solr-6.4.2\example\exampledocs\post.jar"'

TELEGRAM_API_TOKEN1 = '231161869:AAFpafehgQl9V-5f6-1KvwjPkzhbgdqDflU'  # OpenFinData
TELEGRAM_API_TOKEN2 = '250645074:AAF4vfI4wY177VWQYNzPBAt-JYFVyAWyn1I'
TELEGRAM_API_TOKEN3 = '256228310:AAHUrblIlUY2Py-yXEIlajP4y7Wc3BArnHE'  # SvetaBot
TELEGRAM_API_TOKEN_FINAL = '242536845:AAELC6ay-SVnaw0nIZHCZpsqw_KN8yiaD1U'  # Datatron

FFMPEG_PATH_SERVER = r'C:\Users\imon\Documents\Git\OpenFinData\ffmpeg\bin\ffmpeg.exe'
FFMPEG_PATH_DIMA = r'C:\Users\User\PycharmProjects\OpenFinData\ffmpeg\ffmpeg.exe'
FFMPEG_PATH_MASHA = r'C:\Users\The Cat Trex\PycharmProjects\OpenFinData\ffmpeg\bin\ffmpeg.exe'

CNTK_SERVER_PATH=r'C:/Users/imon/Documents/Git/Datatron 2.0/OpenFinData/cntk'